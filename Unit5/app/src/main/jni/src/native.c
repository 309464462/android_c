...
JNIEXPORT void JNICALL Java_com_example_elvin_unit5_jni_TestJni_startToShowLog
  ...
  void getUIdAndTid(){
      uid_t uid;
      uid = getuid();
      MY_LOG_VERBOSE("Applicaton user id is %u...",uid);

      gid_t  gid;
      gid = getgid();
      MY_LOG_VERBOSE("Applicaton user group id is %u...",gid);
      //获取应用的名字
      char * username;

      username = getlogin();
      MY_LOG_VERBOSE("Applicaton user name is %s...",username);
  }
